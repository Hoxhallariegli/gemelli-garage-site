<tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/50 transition-none border-b border-gray-50 dark:border-gray-700/50 last:border-none">
    ...
    <td class="flex items-center gap-3">
         @if($item->bodyType?->image)
                        <div class="size-10 rounded-xl bg-gray-50 dark:bg-gray-900 p-1 shrink-0 flex items-center justify-center border border-gray-100 dark:border-gray-700">
                            <img src="{{ asset($item->bodyType->image) }}" class="w-full h-auto object-contain">
                        </div>
                    @endif
        <span class="text-[10px] text-gray-400 uppercase font-black tracking-widest">{{ $item->bodyType?->name ?? '-' }}</span>
    </td>
    <td class="px-6 py-5 text-gray-600 dark:text-gray-300 font-bold">{{ $item->wrap_meters_needed }} m</td>
