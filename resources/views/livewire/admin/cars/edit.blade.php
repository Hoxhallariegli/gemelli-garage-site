<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('cars.Edit Car') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('cars.Update info') }}</x-short-description></div><x-back-btn route="admin.cars.index" /></div>
...
<div><x-form.input name="color" type="text" wire:model="color" :label="__('cars.Color')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('cars.Update') }}</x-button></div></form></div>
</div>
