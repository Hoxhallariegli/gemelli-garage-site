<tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/50 transition-none border-b border-gray-50 dark:border-gray-700/50 last:border-none">
    <td class="px-4 py-3">
        <div class="size-10 rounded-xl bg-gray-50 dark:bg-gray-900 flex items-center justify-center overflow-hidden border border-gray-100 dark:border-gray-700">
            ...
            @else
                <x-heroicon-o-cog-6-tooth class="size-5 text-gray-300" />
            @endif
        </div>
    </td>
    ...
    <td class="px-4 py-3">
        <div class="flex items-center gap-2">
            @if($item->stock_quantity <= 0)
                <span class="px-2.5 py-1 text-[9px] font-black uppercase tracking-widest bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded-lg">{{ __('parts.Out of Stock') }}</span>
            @elseif($item->stock_quantity < 5)
                <span class="px-2.5 py-1 text-[9px] font-black uppercase tracking-widest bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-lg">{{ $item->stock_quantity }} {{ __('parts.pcs') }} ({{ __('parts.Low') }})</span>
            @else
                <span class="px-2.5 py-1 text-[9px] font-black uppercase tracking-widest bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded-lg">{{ $item->stock_quantity }} {{ __('parts.pcs') }}</span>
            @endif
        </div>
    </td>
    <td class="px-4 py-3 text-right !transition-none">
        <div class="flex justify-end gap-3 !transition-none">
            @can('edit_parts')
                <x-a href="{{ route('admin.parts.edit', $item) }}" class="!rounded-xl !bg-blue-50 dark:!bg-blue-900/30 !text-blue-600 dark:!text-blue-400 !px-3 !py-1 !text-[9px] !font-black !uppercase !border-none">Edit</x-a>
            @endcan
            @can('delete_parts')
