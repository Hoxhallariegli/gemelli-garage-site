<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('clients.Edit Client') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('clients.Update info') }}</x-short-description></div><x-back-btn route="admin.clients.index" /></div>
    ...
    <x-card>
        <form wire:submit.prevent="update" class="space-y-8">
            ...
            <div class="mt-10 flex justify-end">
                <x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('clients.Update') }}</x-button>
            </div>
        </form>
    </x-card>
</div>
