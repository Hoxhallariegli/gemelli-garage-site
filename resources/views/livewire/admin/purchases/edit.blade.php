<div class="space-y-6 md:space-y-10">
    <div class="flex items-center justify-between gap-4 px-1">
        <div>
            <x-h1>{{ __('purchases.Edit Purchase') }}</x-h1>
            <x-short-description class="dark:text-gray-400">{{ __('purchases.Edit Description') }}</x-short-description>
        </div>
    ...
    <form wire:submit.prevent="update" spellcheck="false" autocomplete="off" class="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-10 items-start">
        {{-- Main Section: Items --}}
        <div class="xl:col-span-2 space-y-6 md:space-y-8">
            ...
            <div class="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-gray-50 dark:border-gray-700">
                <div class="flex items-center justify-between mb-8 ml-1">
                    <div class="text-[9px] md:text-[10px] font-black uppercase text-blue-600 tracking-widest md:tracking-[0.2em]">{{ __('purchases.Supply Information') }}</div>
                    <span class="px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest {{ $item->status == 'received' ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500' }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div>
                        <div class="flex items-end gap-2">
                            <div class="flex-1"><x-form.dropdown-search name="supplier_id" wire:model.live="supplier_id" :label="__('purchases.Supplier')" :data="$suppliers" :disabled="$item->status == 'received'" /></div>
                    ...
                    <div><x-form.input name="purchase_date" type="date" wire:model="purchase_date" :label="__('purchases.Purchase Date')" class="dark:bg-gray-900" :disabled="$item->status == 'received'" /></div>
                    <div><x-form.input name="reference_number" type="text" wire:model="reference_number" :label="__('purchases.Reference Number')" class="dark:bg-gray-900" :disabled="$item->status == 'received'" /></div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start bg-gray-50 dark:bg-gray-900/50 p-6 rounded-[2rem] border border-gray-100 dark:border-gray-700 mb-8">
                    <div class="md:col-span-2 space-y-1">
                        ...
                        <select wire:model.live="temp_item_type" class="block w-full bg-white dark:bg-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-700 rounded-2xl py-3 px-4 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-none sm:text-sm font-bold shadow-sm">
                            <option value="Material">{{ __('purchases.Material') }}</option>
                            <option value="Part">{{ __('purchases.Part') }}</option>
                        </select>
                    </div>
                    <div class="md:col-span-4" wire:key="item-dropdown-container-{{ $temp_item_type }}">
                        <div class="flex items-end gap-2">
                            <div class="flex-1">
                                <x-form.dropdown-search wire:key="item-dropdown-{{ $temp_item_type }}" name="temp_item_id" wire:model.live="temp_item_id" :label="__('purchases.Select Item')" :data="$availableItems" />
                            </div>
                    ...
                    <div class="md:col-span-2 flex items-center h-full pt-1.5">
                        <button type="button" wire:click="addItem" wire:loading.attr="disabled" wire:target="addItem" class="w-full bg-blue-600 text-white py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-blue-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                            <span wire:loading.remove wire:target="addItem">{{ __('purchases.Add') }}</span>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-gray-50 dark:border-gray-700">
                ...
                <div class="space-y-6">
                    ...
                    <div class="pt-6 border-t border-gray-100 dark:border-gray-700">
                        <span class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('purchases.Final Total') }}</span>
                        <div class="text-4xl font-black text-gray-900 dark:text-white">€{{ number_format($total_amount, 2) }}</div>
                    </div>

                    @if($item->status === 'pending')
                    <div class="pt-8">
                        <x-button type="submit" variant="blue" wire:loading.attr="disabled" wire:target="update" class="!w-full !py-5 !rounded-2xl !text-sm shadow-xl shadow-blue-500/20 hover:scale-[1.02] transition-all">
                             <span wire:loading.remove wire:target="update">{{ __('purchases.Update Supply') }}</span>
                    ...
                    @else
                    <div class="pt-8 p-6 bg-green-50 dark:bg-green-900/10 rounded-2xl border border-green-100 dark:border-green-800/30 text-center">
                        <x-heroicon-s-check-circle class="w-10 h-10 text-green-500 mx-auto mb-3" />
                        <p class="text-[11px] font-black uppercase text-green-600 tracking-widest">{{ __('purchases.Stock Received Info') }}</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-blue-100 dark:border-blue-900/30 relative overflow-hidden group">
                <div class="relative z-10">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <h4 class="text-[10px] font-black uppercase tracking-widest text-blue-600">{{ __('purchases.Pending Receipt') }}</h4>
                    </div>
                    <p class="text-[11px] leading-relaxed text-gray-500 dark:text-gray-400 font-bold">{{ __('purchases.Pending Receipt Description') }}</p>
                </div>
            </div>
            @endif
        </div>
