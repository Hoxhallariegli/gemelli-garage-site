...
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->brand }}</td>
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->model }}</td>
<td class="px-6 py-5 font-bold text-gray-900 dark:text-white">{{ $item->bodyType?->name ?? '-' }}</td>
<td class="px-6 py-5 font-bold text-gray-900 dark:text-white">{{ $item->service?->name ?? '-' }}</td>
<td class="px-6 py-5 font-bold text-gray-900 dark:text-white">{{ $item->material?->name ?? '-' }}</td>
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->estimated_price }}</td>
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->message }}</td>
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->status }}</td>
    <td class="px-6 py-5 text-right !transition-none">
        <div class="flex justify-end gap-3 !transition-none">
            @can('edit_job_requests')
                <livewire:admin.job-requests.convert-action :job-request="$item" :wire:key="'convert-'.$item->id" />
                <x-a href="{{ route('admin.job-requests.edit', $item) }}" class="!rounded-xl !bg-blue-50 dark:!bg-blue-900/30 !text-blue-600 dark:!text-blue-400 !px-4 !py-1.5 !text-[10px] !font-black !uppercase !border-none">Edit</x-a>
            @endcan
            @can('delete_job_requests')
                <div x-data="{ confirmation: '' }" x-cloak class="inline-block">
                    <x-modal>
                        ...
                        <x-slot name="footer"><x-button variant="gray" @click="on = false">Cancel</x-button><x-button variant="red" x-bind:disabled="confirmation !== '{{ $item->name }}'" wire:click="$parent.deleteJobRequest('{{ $item->id }}')" @click="on = false">Delete</x-button></x-slot>
                    </x-modal>
                </div>
            @endcan
        </div>
