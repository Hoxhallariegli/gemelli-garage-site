<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-1">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <x-h1>{{ __('jobs.Job Card #') }}{{ $item->id }}</x-h1>
                <span class="px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest {{ $status == 'completed' ? 'bg-green-500/10 text-green-500' : 'bg-blue-500/10 text-blue-500' }}">
                    {{ __('jobs.' . $status) ?? $status }}
                </span>
            </div>
            <x-short-description>{{ __('jobs.Update Description') }}</x-short-description>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700">
                <div class="text-[9px] md:text-[10px] font-black uppercase text-blue-600 tracking-widest md:tracking-[0.2em] mb-6 ml-1">{{ __('jobs.Update Job') }}</div>
                ...
                <form wire:submit.prevent="update" class="space-y-4">
                    ...
                    <div class="pt-6 border-t border-gray-100 dark:border-gray-700 space-y-3">
                        ...
                        <div class="pt-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-3xl">
                            ...
                            <div class="text-4xl font-black text-gray-900 dark:text-white italic tracking-tighter leading-none mb-0">€{{ number_format($totalRevenue, 0) }}</div>
                        </div>
                    </div>

                    <x-button type="submit" variant="blue" wire:loading.attr="disabled" wire:target="update" class="!w-full !py-4 !rounded-2xl !text-xs !font-black !uppercase shadow-xl shadow-blue-500/10 mt-4">
                        <span wire:loading.remove wire:target="update">{{ __('jobs.Update Job Card') }}</span>
