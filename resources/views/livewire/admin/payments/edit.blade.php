...
<div><x-form.input name="payment_date" type="datetime-local" wire:model="payment_date" :label="__('payments.Payment Date')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('payments.Update') }}</x-button></div></form></div>
</div>
