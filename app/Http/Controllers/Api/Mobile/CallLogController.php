...
class CallLogController extends Controller
{
    public function index() { abort_if_cannot('view_call_logs'); $items = CallLog::query()->latest()->paginate(50); $items->getCollection()->transform(fn($i) => $this->transformItem($i)); return response()->json($items); }
        ...
        public function update(Request $request, $id, UpdateCallLogAction $action) { abort_if_cannot('edit_call_logs'); $item = CallLog::findOrFail($id); $data = $this->prepareData($request); if(!isset($data[''])) $data[''] = $item->; $dto = CallLogDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
