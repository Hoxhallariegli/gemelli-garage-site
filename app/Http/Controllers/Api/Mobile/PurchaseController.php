...
class PurchaseController extends Controller
{
  ...
  0 => 'supplier',
        ...
        public function update(Request $request, $id, UpdatePurchaseAction $action) { abort_if_cannot('edit_purchases'); $item = Purchase::findOrFail($id); $data = $this->prepareData($request); if(!isset($data[''])) $data[''] = $item->; $dto = PurchaseDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
