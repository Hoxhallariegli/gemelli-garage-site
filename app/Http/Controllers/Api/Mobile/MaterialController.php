...
class MaterialController extends Controller
{
  ...
  0 => 'materialBrand',
        ...
        public function update(Request $request, $id, UpdateMaterialAction $action) { abort_if_cannot('edit_materials'); $item = Material::findOrFail($id); $data = $this->prepareData($request); $dto = MaterialDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
