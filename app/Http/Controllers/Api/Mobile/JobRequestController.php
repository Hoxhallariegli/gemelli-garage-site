...
class JobRequestController extends Controller
{
  ...
  2 => 'material',
        ...
        public function update(Request $request, $id, UpdateJobRequestAction $action) { abort_if_cannot('edit_job_requests'); $item = JobRequest::findOrFail($id); $data = $this->prepareData($request); if(!isset($data[''])) $data[''] = $item->; $dto = JobRequestDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
