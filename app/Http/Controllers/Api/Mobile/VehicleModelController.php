...
class VehicleModelController extends Controller
{
    ...
    public function index(Request $request, VehicleModelListQuery $listQuery)
    {
    ...
    public function store(Request $request, CreateVehicleModelAction $action)
    {
        abort_if_cannot('add_vehicle_models');
        $data = $this->prepareData($request);
        $dto = VehicleModelDTO::fromArray($data);
        $item = $action->execute($dto);
        return response()->json(['success' => true, 'data' => $this->transformItem($item)]);
    }
    
    public function update(Request $request, $id, UpdateVehicleModelAction $action)
    {
    ...
    public function destroy($id, DeleteVehicleModelAction $action)
    {
