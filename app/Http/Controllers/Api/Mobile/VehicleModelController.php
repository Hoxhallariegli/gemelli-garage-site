...
class VehicleModelController extends Controller
{
  ...
  1 => 'bodyType',
        ...
        public function update(Request $request, $id, UpdateVehicleModelAction $action) { abort_if_cannot('edit_vehicle_models'); $item = VehicleModel::findOrFail($id); $data = $this->prepareData($request); $dto = VehicleModelDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
