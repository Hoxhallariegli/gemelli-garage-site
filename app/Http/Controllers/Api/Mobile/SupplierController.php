...
class SupplierController extends Controller
{
    public function index() { abort_if_cannot('view_suppliers'); $items = Supplier::query()->latest()->paginate(50); $items->getCollection()->transform(fn($i) => $this->transformItem($i)); return response()->json($items); }
        ...
        public function update(Request $request, $id, UpdateSupplierAction $action) { abort_if_cannot('edit_suppliers'); $item = Supplier::findOrFail($id); $data = $this->prepareData($request); if(!isset($data[''])) $data[''] = $item->; $dto = SupplierDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
