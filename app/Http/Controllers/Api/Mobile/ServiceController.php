...
class ServiceController extends Controller
{
    public function index() { abort_if_cannot('view_services'); $items = Service::query()->latest()->paginate(50); $items->getCollection()->transform(fn($i) => $this->transformItem($i)); return response()->json($items); }
        ...
        public function update(Request $request, $id, UpdateServiceAction $action) { abort_if_cannot('edit_services'); $item = Service::findOrFail($id); $data = $this->prepareData($request); $dto = ServiceDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
