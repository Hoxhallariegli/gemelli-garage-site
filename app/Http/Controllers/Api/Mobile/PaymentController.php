...
class PaymentController extends Controller
{
  ...
  0 => 'job',
        ...
        public function update(Request $request, $id, UpdatePaymentAction $action) { abort_if_cannot('edit_payments'); $item = Payment::findOrFail($id); $data = $this->prepareData($request); $dto = PaymentDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
