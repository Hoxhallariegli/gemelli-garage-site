...
class CarController extends Controller
{
  ...
  2 => 'model',
        ...
        public function update(Request $request, $id, UpdateCarAction $action) { abort_if_cannot('edit_cars'); $item = Car::findOrFail($id); $data = $this->prepareData($request); $dto = CarDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
