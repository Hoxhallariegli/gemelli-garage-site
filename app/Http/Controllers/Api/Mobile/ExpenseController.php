...
class ExpenseController extends Controller
{
  ...
  0 => 'job',
        ...
        public function update(Request $request, $id, UpdateExpenseAction $action) { abort_if_cannot('edit_expenses'); $item = Expense::findOrFail($id); $data = $this->prepareData($request); if(!isset($data[''])) $data[''] = $item->; $dto = ExpenseDTO::fromArray($data); $item = $action->execute($item, $dto); return response()->json(['success' => true, 'data' => $this->transformItem($item)]); }
